...
> 레거시 별칭(`cast sync`, `cast ls`, `cast rm`)은 deprecated 상태이며 다음 메이저 버전에서 제거됩니다.
...
`--json`은 모든 명령에서 지원되며 항상 `{ ok, data }` 또는 `{ ok, error, message }` 형태를 반환합니다.
